...
# === Load Vector Database with Optimized FAISS ===
embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2", model_kwargs={'device': 'cpu'})
vector_db_path = "second_vector_db"
vector_db = FAISS.load_local(vector_db_path, embeddings, allow_dangerous_deserialization=True)
vector_db.index.nprobe = 10  # Optimize FAISS Search

def retrieve_context(query, top_k=3):
    """Retrieves the most relevant documents from FAISS."""
    try:
...
hf_pipeline = pipeline(
    ...
    batch_size=4  # Increase batch size for parallel inference
)
...
def generate_response(question):
    """Generates a response using the LLM with performance optimization."""
    try:
        ...
        with torch.inference_mode():  # Disable gradient for faster inference
...
for i, question in enumerate(QUESTIONS):
    ...
    if result:
        ...
        print(f"API Time Taken: {total_time} sec")
